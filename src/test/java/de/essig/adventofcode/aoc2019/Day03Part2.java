...
public class Day03Part2 {
    ...
    private String testInput2 = "R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51\n"
        + "U98,R91,D20,R16,D67,R40,U7,R15,U6,R7";

    private String testInputJulia = "R8,U5,L5,D3\n"
        + "U7,R6,D4,L4";
    ...
    private Map<Point, Integer> pointsWireOne = new HashMap<>();
    private Map<Point, Integer> pointsWireTwo = new HashMap<>();

    @Test
    void runPuzzle() {

        System.out.println("Smallest Dist: " + calculateSteps(puzzleInput));
    }
    ...
    void runTests() {
        ...
        assertThat(calculateSteps(testInputJulia), is(30));

        assertThat(calculateSteps(testInput1), is(610));
        assertThat(calculateSteps(testInput2), is(410));
    }
    ...
    public int calculateSteps(String input) {
        ...
        String[] wireOne = lines[0].split(",");
        String[] wireTwo = lines[1].split(",");

        pointsWireOne = calculatePoints(wireOne);
        pointsWireTwo = calculatePoints(wireTwo);

        // System.out.println("Points One: " + pointsWireOne);
        // System.out.println("Points Two: " + pointsWireTwo);

        Set<Point> commonPoints = pointsWireOne.keySet();
        commonPoints.retainAll(pointsWireTwo.keySet());

        System.out.println("Common Points: " + commonPoints);

        // Remove Start:
        commonPoints.remove(new Point(0, 0));

        int smaltestDist = Integer.MAX_VALUE;
        Point nearestPoint = null;

        for (Point p : commonPoints) {
            int dist = pointsWireOne.get(p) + pointsWireTwo.get(p);
            System.out.println("Steps: " + dist);

            if (dist < smaltestDist) {
                smaltestDist = dist;
                nearestPoint = p;
            }
        }

        System.out.println("Best Point" + nearestPoint);

        return smaltestDist;
    }


    private Map<Point, Integer> calculatePoints(String[] directions) {

        Map<Point, Integer> points = new HashMap<>();
        int lastX = 0;
        int lastY = 0;
        int steps = 1; // Off by one -> The start sqaure must not be removed from the number of steps

        for (String command : directions) {
            // System.out.println("Dir: " + command + " at Pos: " + lastX + "/" + lastY);
            int dist = parseDistance(command);
            Direction dir = parseDirection(command);
            steps--; // Off by one -> Do not count the current square twice

            if (dir == Direction.RIGHT) {
                for (int x = lastX; x <= lastX + dist; x++) {
                    points.put(new Point(x, lastY), steps++);
                }
                ...
                for (int y = lastY; y <= lastY + dist; y++) {
                    points.put(new Point(lastX, y), steps++);
                }
    ...
    @Test
    void testParseDistance() {

        assertThat(parseDistance("R23"), is(23));
    }
    ...
    void testParseDir() {
        ...
        assertThat(parseDirection("R23"), is(Direction.RIGHT));
    }


    private int parseDistance(String direction) {

        return Integer.valueOf(direction.substring(1));
    }
    ...
    private Direction parseDirection(String direction) {
        ...
        switch (direction.substring(0, 1)) {
            ...
            default:
                System.err.println("Error: Unknown direction in " + direction);
                System.exit(-1);
