...
public class Day03Part1 {
    ...
    private Set<Point> pointsWireOne = new HashSet<>();
    private Set<Point> pointsWireTwo = new HashSet<>();

    private enum Direction {RIGHT, DOWN, LEFT, UP};

    @Test
    void runPuzzle() {
        System.out.println("Smallest Dist: " + calculate(puzzleInput));
    }

    @Test
    void runTests() {
        assertThat(calculate(testInput1), is(159));
        assertThat(calculate(testInput2), is(135));
    }
    ...
    public int calculate(String input) {
        ...
        pointsWireOne.retainAll(pointsWireTwo);

        //System.out.println("Common Points: " + pointsWireOne);

        // Remove Start:
        pointsWireOne.remove( new Point(0,0));

        int smaltestDist = Integer.MAX_VALUE;
        Point nearestPoint = null;

        for(Point p : pointsWireOne) {
            int dist = Math.abs(p.getX()) + Math.abs(p.getY());
    ...
    private Set<Point> calculatePoints(String[] directions) {
